...
public class State_Controller_UMP_Overlay : SingletonBehaviour<State_Controller_UMP_Overlay>
{
    public FB_Controller_UMP FB_datebase;
    public Controller_UMP_Overlay ControllerVP;
    ...
    private ManagerSelecteCurrentMovie_UMP SelectedManager;
    private Context_StrategySelectedMovies_UMP contextStrategySelectedMovies;
    ...
    void Start()
    {
        ...
        SelectedManager = new ManagerSelecteCurrentMovie_UMP(new StrategyMainPlayList_UMP());
        contextStrategySelectedMovies = new Context_StrategySelectedMovies_UMP(new StateStategy_Internet_Is_available_UMP());
    ...
    public void InternetStatus()
    {
        if (Application.internetReachability != NetworkReachability.NotReachable)
        {
            ...
            contextStrategySelectedMovies.FindOut(MarkStrategy_UMP.Internet_Is_available);
        ...
        else
        {
            ...
            if (check)
            {
                contextStrategySelectedMovies.FindOut(MarkStrategy_UMP.Internet_No_Full_PlayList);

            }
            else
            {
                contextStrategySelectedMovies.FindOut(MarkStrategy_UMP.Internet_No_Short_PlayList);
    ...
    public void SetPathForPlayer(string path, float offset, bool url)
    {
        Debug.Log("SEEK_1" + offset);
        ControllerVP.ReplacePlayVideo(path, offset, url);
    }

    public Controller_UMP_Overlay GetSimpleController()
    {
        return ControllerVP;
    }
    ...
    public ManagerSelecteCurrentMovie_UMP GetStrategyselectedMovies()
    {
    ...
    public void StartDowloadMoive()
    {
        ...
        else
        {
            ...
            if (ControllerVP._mediaPlayer.IsPlaying && !DataSchedule.Instance.GetDataschedules()[SelectedManager.currentNumberMovie].isLocal && DataSchedule.Instance.GetDataschedules()[SelectedManager.currentNumberMovie].PathLocal != null)
            {
                Debug.Log("Point_2");
                DateTime localDate = DateTime.Now;
                int mSec = (Int32.Parse(localDate.ToString("HH")) * 3600 + Int32.Parse(localDate.ToString("mm")) * 60 + Int32.Parse(localDate.ToString("ss"))) * 1000;
    ...
    void LoaderMovies()
    {
        ...
        StartCoroutine(_CoroutinaLoadGlobal_URL((isLoad_URL) =>
        {
            ItemLoadMovie_UMP itemLoadMovie = new ItemLoadMovie_UMP();
            itemLoadMovie.pathToGlobalPLayList = DataSchedule.Instance.GetDataschedules()[numberCurrentDowloadClip].PathLoadGlobal;
    ...
    private IEnumerator _CoroutinaLoadGlobal_URL(Action<bool> action)
    {
    ...
    private IEnumerator _CoroutinaLoadGlobalPlayList(ItemLoadMovie_UMP item, Action<bool> action)
    {
    ...
    IEnumerator _CoroutinaLoadLocalPlayList(ItemLoadMovie_UMP item, Action<bool> action)
    {
        bool endLoad = false;
        while (!endLoad)
        {
    ...
    void WritePlaylistM3U8(ItemLoadMovie_UMP item)
    {
        try
        {
    ...
    IEnumerator _CoroutinaLoadSegment(ItemLoadMovie_UMP item, Action<bool> action)
    {
    ...
    private void SaveLocalPathToMovies(ItemLoadMovie_UMP item)
    {
        if (!StopDowloadMoive)
        {
    ...
    private void SelectedNumberLoadedMovie(ItemLoadMovie_UMP item)
    {
        if (!StopDowloadMoive)
        {
            //numberDowloadClip++;
    ...
    private void DeleteFolderMovie(ItemLoadMovie_UMP item)
    {
        ...
        if (!shedule.HasError)
        {
            ...
            if (response.ContainsKey("data"))
            {
                ...
                if (response["data"]["schedule"].ContainsKey("mediasIdList"))
                {
                    ...
                    if (Item!=null)
                    {
                        if (string.IsNullOrEmpty(DataSchedule.Instance.GetDataschedules()[Item.number].PathLoad))
                        {
                            string id = DataSchedule.Instance.GetDataschedules()[Item.number].id;
                            string path = null;
                            // Debug.Log("PathLoad=" + ItemClip["path"].ToString());
                            WWW www_PlayList = new WWW(DataSchedule.Instance.GetDataschedules()[Item.number].PathLoadGlobal);
